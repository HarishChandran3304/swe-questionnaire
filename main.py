...
def main():
    ...
    if st.button("Submit"):
        if "" in [age, gender, birth_order, education, family_type] or "" in responses.values() or None in responses.values():
            st.error("Please answer all questions before submitting.")
        else:
            row = [datetime.now().strftime("%Y-%m-%d %H:%M:%S"), age, gender, birth_order, education, family_type] + list(responses.values())
            insert_row(row)
